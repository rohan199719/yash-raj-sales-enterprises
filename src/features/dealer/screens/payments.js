...
export default function Payments({ navigation }) {
  ...
  const [exportPaymentOverlayOpenFlag, setExportPaymentOverlayOpenFlag] = useState(false);
...
paymentHistorydummy.push({ dealerName: 'John Doe', dueAmount: 100, paymentDate: '2023-05-01' });
paymentHistorydummy.push({ dealerName: 'Jane Smith', dueAmount: 200, paymentDate: '2023-05-05' });
// ... add more payment objects as needed
      ...
      <table>
        ...
        <tbody>
          ...
          <td>${payment.dealerName}</td>
            <td>${months[payment.paymentDateTimestamp.toDate().getMonth()]}${" "}${payment.paymentDateTimestamp.toDate().getDate().toString()}${" "}${payment.paymentDateTimestamp.toDate().getFullYear().toString()}</td>
            <td>${payment.paymentAmount}</td>
            <td>${payment.recievedBy}</td>
            <td>${payment.paymentMode}</td>
          </tr>`).join(''):paymentHistorydummy.map((payment) => `
          <tr>
            <td>${payment.dealerName}</td>
            <td>${payment.paymentAmount}</td>
  ...
  return (
    isLoading? <ActivityIndicator size="large" color="#689F38" style={{position:"absolute",right:'50%',left:'50%',top:'50%',bottom:'50%'}}/> :<View style={{height:'100%'}}>
      {paymentHistory && <PaymentListView>
